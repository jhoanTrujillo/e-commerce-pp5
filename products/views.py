...
def all_products(request):
    ...
    # Specify the current template name
    current_template = 'products/products.html'
...
def product_details(request, product_id):
    ...
    recommended_products = Product.objects.filter(collection=1)
...
def add_product_or_variant(request):
    ...
    if not request.user.is_superuser:
        messages.failed(request, 'Your account don\'t \
            count with the permission required')
    ...
    if request.method == 'POST':
        ...
        if variant_form.is_valid():
            ...
            return redirect('variant_details',
            ...
            messages.error(request, 'Failed to add product or variant.\
...
def edit_product_or_variant(request, item_id):
    ...
    if not request.user.is_superuser:
        messages.failed(request, 'Your account don\'t count\
            with the permission required')
    ...
    if request.method == 'POST':
        if is_variant:
            form = VariantForm(request.POST,
                               request.FILES, instance=product_or_variant)
        else:
        ...
        if form.is_valid():
            ...
            if is_variant:
                ...
                return redirect(reverse('variant_details',
                                        args=[product_id, variant_id]))
            else:
                return redirect('product_details',
                                product_id=product_or_variant.id)
        else:
...
def delete_product_or_variant(request, item_id):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Your account doesn\'t have\
    ...
    try:
        ...
        messages.error(request, 'Unable to delete product or variant.\
            Please try again.')
        print(f"Exception: {e}")
